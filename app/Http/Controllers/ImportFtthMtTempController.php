        ...
        if ($request->hasFile('fileFtthMT')) {
            ...
            try {
                ...
                $doneSortir = DB::table('import_ftth_mt_temps')->where('status_wo', '=', 'Done')
                    ->select(DB::raw('no_wo,max(tgl_ikr) as tgl_ikr, status_wo'))
                    ->where('type_wo','=','MT')
                    ->orderBy('no_wo')
                    ->groupBy('no_wo','status_wo')
                    ->get();
                ...
                $pendingSortir = DB::table('import_ftth_mt_temps')->where('status_wo', '=', 'Pending')
                    ...
                    ->whereNotIn('no_wo', function ($p) {
                        $p->select('no_wo')->distinct()->from('import_ftth_mt_temps as import1')->whereIn('status_wo',['Done','Cancel']);
                    })
                ...
                $cancelSortir = DB::table('import_ftth_mt_temps')->where('status_wo', '=', 'Cancel')
                    ...
                    ->where('type_wo','=','MT')
                    ->whereNotIn('no_wo', function ($p) {
                        $p->select('no_wo')->distinct()->from('import_ftth_mt_temps as import1')->where('status_wo', '=', 'Done');
                    })
                    // ->whereNotIn('no_wo', function ($p) {
                    //     $p->select('no_wo')->from('import_ftth_mt_temps as import1')->where('status_wo', '=', 'Pending');
                    // })
                    ->orderBy('no_wo')
                    ->groupBy('no_wo','status_wo')
                    ->get();

                // dump($doneSortir,$pendingSortir, $cancelSortir);
                ...
                for ($x = 0; $x < $donePendingCancelSortir->count(); $x++) {
                    ...
                    $importFtthMtOri = ImportFtthMtTemp::where('no_wo', '=', $donePendingCancelSortir[$x]->no_wo)
                        ->where('tgl_ikr', '=', $donePendingCancelSortir[$x]->tgl_ikr)
                        ->where('status_wo','=', $donePendingCancelSortir[$x]->status_wo)->first();
                    ...
                    ImportFtthMtSortirTemp::create([
                        ...
                        'tgl_reschedule' => $importFtthMtOri->tgl_reschedule,
                        'tgl_jam_reschedule' => $importFtthMtOri->tgl_jam_reschedule,
                        'alasan_cancel' => $importFtthMtOri->alasan_cancel,
                        'alasan_pending' => $importFtthMtOri->alasan_pending,
                        'detail_alasan' => $importFtthMtOri->detail_alasan,
                        'permintaan_rsch' => $importFtthMtOri->permintaan_rsch,
                        'respon_cst' => $importFtthMtOri->respon_cst,
                        'jawaban_cst' => $importFtthMtOri->jawaban_cst,
                        'dispatch' => $importFtthMtOri->dispatch,
                        'telp_dispatch' => $importFtthMtOri->telp_dispatch,
                        ...
                        'foto_rumah' => $importFtthMtOri->foto_rumah,
                        'foto_selfie' => $importFtthMtOri->foto_selfie,
                        'regist_start' => $importFtthMtOri->regist_start,
                        'regist_end' => $importFtthMtOri->regist_end,
                        'cek_telebot' => $importFtthMtOri->cek_telebot,
                        'hasil_cek_telebot' => $importFtthMtOri->hasil_cek_telebot,
                        ...
                        'pic_konf_cst' => $importFtthMtOri->pic_konf_cst,
                        'konfirmasi_customer' => $importFtthMtOri->konfirmasi_customer,
                        'tgl_konf_cst' => $importFtthMtOri->tgl_konf_cst,
                        'jam_konf_cst' => $importFtthMtOri->jam_konf_cst,
                        'bukti_konf_cst' => $importFtthMtOri->bukti_konf_cst,
                        ...
                        'kabel_utp' => $importFtthMtOri->kabel_utp,
                        'pipa' => $importFtthMtOri->pipa,
                        'socket_pipa' => $importFtthMtOri->socket_pipa,
                        'cable_duct' => $importFtthMtOri->cable_duct,
                        'rj45' => $importFtthMtOri->rj45,
                        ...
                        'created_at' => $importFtthMtOri->created_at,
                        'updated_at' => $importFtthMtOri->updated_at,
                        'wo_type_apk' => $importFtthMtOri->wo_type_apk,
                        'branch_id' => $importFtthMtOri->branch_id,
                        'leadcall' => $importFtthMtOri->leadcall,
                        'tek1_nik' => $importFtthMtOri->tek1_nik,
                        'tek2_nik' => $importFtthMtOri->tek2_nik,
                        'tek3_nik' => $importFtthMtOri->tek3_nik,
                        'tek4_nik' => $importFtthMtOri->tek4_nik,
                        'leadcall_id' => $importFtthMtOri->leadcall_id,
                        'leader_id' => $importFtthMtOri->leader_id,
                        'callsign_id' => $importFtthMtOri->callsign_id,
                        'teknisi4' => $importFtthMtOri->teknisi4,
                        'kondisi_fat' => $importFtthMtOri->kondisi_fat,
                        'alasan_tidak_ganti_precon' => $importFtthMtOri->alasan_tidak_ganti_precon,
                        'is_checked' => $importFtthMtOri->is_checked,
                        'mttr_all' => $importFtthMtOri->mttr_all,
                        'mttr_pending' => $importFtthMtOri->mttr_pending,
                        'mttr_progress' => $importFtthMtOri->mttr_progress,
                        'mttr_teknisi' => $importFtthMtOri->mttr_teknisi,
                        'sla_over' => $importFtthMtOri->sla_over,
                        'minute' => $importFtthMtOri->minute,
                        'status_checkin' => $importFtthMtOri->status_checkin,
                        'waktu_installation' => $importFtthMtOri->waktu_installation,
                        'kode_otp' => $importFtthMtOri->kode_otp,
                        'material_out' => $importFtthMtOri->material_out,
                        'material_in' => $importFtthMtOri->material_in,
                        'login' => $akses
                    ]);
                }
